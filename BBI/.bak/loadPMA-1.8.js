/********************************************************************* 
#- Version: 1.8
#- Last Modified: Ted, Thu Aug 27 15:11:56 EDT 2015
ActionFlg List:
PMA_loginChk
PMA_shellCmd
PMA_getDealList
...
var asmType={ppy:{base:"CPR",TypeList:["PSA","CPR","SMM"]},dft:{base:"CDR",TypeList:["SDA","CDR","MDR"]},idx:{base:"LIBOR_1MO",TypeList:["LIBOR_1MO","LIBOR_3MO","LIBOR_6MO"]}};
...
function RUN_getCfs(actionFlg,vObj,idName,p) {
	...
	//		chartTbl(idName,cfsObj);
			if(idName=="cfs") {
				var xcfsNew=PageGrid.cfs.getColumns();
...
function colObjInit(idNum,prfObj,sampleData) {
	...
	if(typeof(prfObj) == "object") {
		...
		nameX=prfObj;
		toolTipX=prfObj;
		precisionX=2; datatypeX=0; showX=1; 
	}
	var colObj = {
		id : idX, field: fieldX, name : nameX, width : 60, 
		toolTip:toolTipX,
		precision : precisionX, datatype : datatypeX, show : showX, colid: idNum, 
		datatype: 0, editor: Slick.Editors.Text
	}
	if(sampleData != null && typeof(sampleData) == "number") {
		colObj.datatype =1;
		colObj["formatter"]=Slick.Formatters.Round2; 
	}
	return colObj;
}
...
function prfDxUpd(dx,datatype){	   // update columns  with additional properties
	if (datatype>0) {
		var formatType=rvmTypeConverter(dx.datatype);
		_.extend(dx, { editor: Slick.Editors.Text,formatter:formatType,validator: requiredFieldValidator});
	} else {
		_.extend(dx, { editor: Slick.Editors.Text,validator: requiredFieldValidator});
	}
}

function gridColAsg(hdrL,dataG,prfN) {
	var clxBas=[];
	var clxCur=[];
	if(prfN==null || prfN=="") prfN=currName;
...
/* test only
	clxBas.splice(3,5); 
	clxBas[1].show=0; clxBas[3].show=0; clxBas[5].show=0;
*/
...
function adjGridCanvasWidth(idName,gridId) {
	if(parseFloat(PageGrid[idName].getCanvasNode().style.height) > parseFloat($(gridId).css("height"))+10)
...
function asmTableUpd() {
	$("#asmTable input,select").change(function(e) {
