...
function viewEdit(yn,prfN) {
	...
	var urlName='_prfViewPop.htm?appName='+selist
		...
		+'&dealType='+dealType
		+'&prfName='+prfN;
	prfViewWin=window.open(urlName,'ViewPreference','resizable=yes,menubar=0,titlebar=1,status=0,width=900,height=640');
}
...
// TO LOAD  jsonData to [tab]Grid as text, such as rle & gnr
					...
					$.each(keyObj.jsonData,function(i,d) {
						tmpStr=tmpStr+d+"\n";});
					document.getElementById(gridName).value=tmpStr;
				}
			}
		}
		for(ky in asmDspList) { asmDcmUpdByType(ky,asmFpList[ky]); }
...
//PMA_getRptList
function RUN_getRptList(actionFlg,loginName,dealName) {
	urlName=urlPath
		+'?actionFlg='+actionFlg
		+'&dealType=' + dealType
		+'&loginName=' + loginName
	...
	$.getJSON(urlName, function(ret) {
		if(ret.length>0 && ret[0]!="" ) 
			loadSelectBox1("rptList",ret,"","(Select Report)");
		else
			loadSelectBox1("rptList",[""],"","(No Report)");
	});
}
...
function prfView_apply(actionFlg,appN,prfN,actN) {
	var clxCur= prfColAsg(PageTbl[prfN+"_OBJ"].jsonHeader,PageGrid[prfN].getData(),prfN);
	PageGrid[prfN].setColumns(clxCur);
}

function prfView_bnd(actionFlg,appN,prfN,actN) {
	prfView_apply(actionFlg,appN,prfN,actN);
}
...
function _dcmGridUpd(x,y,xstr) {
	var o="dcm_"+y;h=y.capitalize()+"ScnX"+x;
